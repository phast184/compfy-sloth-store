...
const CartItem = ({ item }) => {
  const { removeCartItem, toggeCartAmount } = useCartContext();
  const increase = () =>{
    toggeCartAmount(item.id, 'inc')
  }

  const decrease = () => {
    toggeCartAmount(item.id, 'dec')
  }
  return (
    <Wrapper>
      <div className="title">
      ...
      <div className="subtotal">{formatPrice(item.price * item.amount)}</div>
